package com.ccyang._05_BinarySearchTree._01_optional_floor_and_ceil;
...
public class BinarySearch {
     ...
     *  如果找到target，返回第一个target响应的索引
     *  如果没有找到target，返回比这个target小的最大值相应的索引，如果这个最大值有多个，返回最大索引
     *  如果target比整个数组最小的元素值还要小，则不存在这个target的floor值，返回 -1。
     */
    ...
    /**
     *  二分查找法，在有序的数组中，查找target
     *  如果找到target，返回最后一个target响应的索引
     *  如果没有找到target，返回比这个target大的最小值相应的索引，如果这个最小值有多个，返回最小索引
     *  如果target比整个数组最大的元素值还要大，则不存在这个target的ceil值，返回整个数组的元素个数 n 。
     */
    public static int ceil (Comparable[] arr, Comparable target){
        ...
        // 如果该索引 -1就是target本身，该索引 +1 就是即为返回值
